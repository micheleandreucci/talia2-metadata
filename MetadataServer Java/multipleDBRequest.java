...
public class multipleDBRequest {
	...
	/*public void loadDeliverablesData() throws SQLException, ParseException {
		...
		for (Document document : json_object.getDocuments()) {
			...
			db.loadDeliverableData(deliverable.getUrl(), deliverable.getTitle(), deliverable.getDate(), 
	...
	public void loadProjectsData() throws NumberFormatException, SQLException {
		...
		for (Document document : json_object.getDocuments()) {
			...
			db.loadProjectData(project.getAxis(), project.getObjective(), project.getAcronym(), project.getLabel(), project.getSummary(), project.getCall(), project.getStart(),
					project.getEnd(), project.getType(), project.getErdf(), project.getIpa(), project.getAmount(), Double.parseDouble(project.getCofinancing()), project.getStatus(),
					project.getDeliverablesUrl(), json_object.getCollection());
		}
	}
	
	public void loadPartnersData() throws UnsupportedEncodingException, SQLException {
		...
		for (Document document : json_object.getDocuments()) {
			...
			for (Partner partner : project.getPartners()) {
				...
				db.loadPartnerData(partner.isLP(), partner.getName(), partner.getNature(), partner.getCountry(), partner.getPostalCode(), partner.getArea(), partner.getNUTS3(), null,
						null, partner.getErdf(), partner.getErdfContribution(), partner.getIpa(), partner.getIpaContribution(), partner.getAmount());
			}
		}
	}
	...
	public void loadAllJsonData(DocumentListJson json_object) throws SQLException, ParseException, UnsupportedEncodingException, UnknownDeliverableException, NumberFormatException {
	...
	public static void main (String args[]) {
		...
		} catch (SQLException | NumberFormatException | UnsupportedEncodingException | ParseException | UnknownDeliverableException e) {
